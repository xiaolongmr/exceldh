...
window.toggleCustomMode = function () {
    ...
    if (btn) btn.innerHTML = window.isCustomizing ? '<span>💾保存</span>' : '<span>⚙️编辑</span>';
    if (!window.isCustomizing) {
        updateFavsOrder();
        // 只有在排序发生变化时才显示提示
...
// 配置选项：是否在控制台显示日志信息
const config = {
    logToConsole: false // true为显示日志，false为不显示
};
...
function collectAllSitesFromExcel() {
    try {
        ...
        if (!mainContent) {
            logger.log('Excel容器未找到，等待加载...');
            return;
        }

        const cards = mainContent.querySelectorAll('.box-card');
        logger.log('找到卡片数量:', cards.length);

        let sites = [];
        cards.forEach((card, cardIndex) => {
            const items = card.querySelectorAll('a');
            logger.log(`卡片${cardIndex + 1}中的链接数量:`, items.length);
        ...
        logger.log('成功收集站点数据，总数:', sites.length);

        // 如果站点数量为0，可能是Excel数据还没加载完成
        if (sites.length === 0) {
            logger.log('未收集到站点数据，可能Excel还在加载中...');
        }

    } catch (e) {
        window.allSites = [];
        logger.error('收集站点数据失败:', e);
    }
}
