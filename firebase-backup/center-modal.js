...
async function renderProfileCenter(content) {
    ...
    <div style="position:relative;width:900px;max-width:98vw;max-height:96vh;overflow:auto;background:#fff;border-radius:10px;box-shadow:0 2px 12px rgba(0,0,0,0.10);margin:auto;">
      ...
      <div class="profile-cover" style="height:220px;background:rgb(231, 231, 231) url('${userProfileData.coverImage}') center center/cover no-repeat;position:relative;">
        ...
        <button id="change-cover-btn" style="position:absolute;right:24px;bottom:18px;background:rgba(0,0,0,0.45);color:#fff;border:none;border-radius:6px;padding:7px 18px;cursor:pointer;font-size:13px;">修改封面</button>
      </div>
      <div style="padding:70px 32px 0 32px;text-align:center;">
        ...
        <div class="profile-bio" style="color:#888;font-size:13px;margin:10px 0 18px 0;">${userProfileData.bio}</div>
        <div class="profile-stats" style="display:flex;justify-content:center;gap:32px;font-size:13px;color:#666;margin-bottom:18px;">
        ...
        <div style="display:flex;justify-content:center;gap:18px;margin-bottom:24px;">
          <button class="follow-btn" style="background:#3385ff;color:#fff;border:none;border-radius:6px;padding:7px 32px;font-size:13px;cursor:pointer;">${userProfileData.following ? '取消关注' : '+ 关注'}</button>
          <button class="private-msg-btn" style="background:#fff;color:#3385ff;border:1.5px solid #3385ff;border-radius:6px;padding:7px 32px;font-size:13px;cursor:pointer;">私信</button>
        </div>
      </div>
...
function renderLogoutConfirm(content) {
    ...
    <div style="position:relative;width:340px;max-width:98vw;background:#fff;border-radius:10px;box-shadow:0 2px 12px rgba(0,0,0,0.10);margin:auto;padding:36px 32px 32px 32px;text-align:center;">
      ...
      <button id="logout-confirm-btn" style="background:#f44336;color:#fff;border:none;border-radius:6px;padding:8px 32px;font-size:13px;">退出登录</button>
    </div>`;
    document.getElementById('close-center-modal-btn').onclick = closeCenterModal;
...
function showCustomTip(msg, type = 'success') {
    ...
    tip.textContent = msg;
    tip.style = `position:fixed;top:18px;left:50%;transform:translateX(-50%);z-index:10050;padding:10px 32px;background:${type === 'error' ? '#f44336' : '#3385ff'};color:#fff;border-radius:6px;font-size:13px;box-shadow:0 2px 8px rgba(0,0,0,0.12);opacity:0.98;`;
